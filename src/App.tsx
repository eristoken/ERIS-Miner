...
function NavigationTabs() {
  ...
  useEffect(() => {
    ...
    else if (path === '/stats') setValue(1);
    else if (path === '/settings') setValue(2);
    else if (path === '/rpcs') setValue(3);
    else if (path === '/console') setValue(4);
    else if (path === '/about') setValue(5);
  }, [location]);

  const handleChange = (_: React.SyntheticEvent, newValue: number) => {
    setValue(newValue);
    const paths = ['/', '/stats', '/settings', '/rpcs', '/console', '/about'];
